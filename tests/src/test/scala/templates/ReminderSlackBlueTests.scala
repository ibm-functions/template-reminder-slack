...
class ReminderSlackBlueTests extends TestHelpers
  ...
  it should "create the nodejs 8 reminder slack template from github url" in withAssetCleaner(wskprops) { (wp, assetHelper) =>
    ...
    val compValue = JsArray(
      JsString("/" + namespace + "/" + nodejs8SlackAction),
      JsString("/" + namespace + "/" + slackPostAction)
    )
  ...
  it should "create the nodejs 6 reminder slack template from github url" in withAssetCleaner(wskprops) { (wp, assetHelper) =>
    // create unique asset names
    val timestamp: String = System.currentTimeMillis.toString
    ...
    // check that sequence was created and contains correct actions
    val compValue = JsArray(
      JsString("/" + namespace + "/" + nodejs6SlackAction),
      JsString("/" + namespace + "/" + slackPostAction)
    )
  ...
  it should "create the php reminder slack template from github url" in withAssetCleaner(wskprops) { (wp, assetHelper) =>
    // create unique asset names
    val timestamp: String = System.currentTimeMillis.toString
    ...
    // check that sequence was created and contains correct actions
    val compValue = JsArray(
      JsString("/" + namespace + "/" + phpSlackAction),
      JsString("/" + namespace + "/" + slackPostAction)
    )
  ...
  it should "create the python reminder slack template from github url" in withAssetCleaner(wskprops) { (wp, assetHelper) =>
    // create unique asset names
    val timestamp: String = System.currentTimeMillis.toString
    ...
    // check that sequence was created and contains correct actions
    val compValue = JsArray(
      JsString("/" + namespace + "/" + pythonSlackAction),
      JsString("/" + namespace + "/" + slackPostAction)
    )
  ...
  it should "create the swift reminder slack template from github url" in withAssetCleaner(wskprops) { (wp, assetHelper) =>
    // create unique asset names
    val timestamp: String = System.currentTimeMillis.toString
    ...
    // check that sequence was created and contains correct actions
    val compValue = JsArray(
      JsString("/" + namespace + "/" + swiftSlackAction),
      JsString("/" + namespace + "/" + slackPostAction)
    )
